...
export default class ShopData extends foundry.abstract.TypeDataModel {
   ...
   * @param {string} configuration.pageUuid       The uuid of the shop page.
   * @param {string} configuration.stockUuid      The uuid of the entry in the stock.
   * @param {string} configuration.customerUuid   The uuid of the actor making the purchase.
   * @param {number} [configuration.quantity=1]   The quantity to purchase.
   * @returns {Promise<QueryResult>}
   */
  static _query = async ({ pageUuid, stockUuid, customerUuid, quantity = 1 }) => {
    if (!ShopData.#semaphore) {
      ShopData.#semaphore = new foundry.utils.Semaphore(1);
    }

    return ShopData.#semaphore.add(ShopData.#query, { pageUuid, stockUuid, customerUuid, quantity });
  };
   ...
   * @returns {Promise<QueryResult>}
   */
  static #query = async ({ pageUuid, stockUuid, customerUuid, quantity = 1 }) => {
    const page = await foundry.utils.fromUuid(pageUuid);
    const stock = page.system.stock.find(s => s.uuid === stockUuid);
    if (!stock) {
      return {
        success: false,
    ...
    const isStack = quantity === stock.quantity;
    const { value, denomination } = isStack
      ? stock.price.stack
      : { value: (stock.price.each.value * quantity).toNearest(0.1), denomination: stock.price.each.denomination };

    const customer = await foundry.utils.fromUuid(customerUuid);
    ...
    if (item.type === "container") {
      ...
      itemData.push(foundry.utils.mergeObject(game.items.fromCompendium(item), { "system.quantity": quantity }));
    }

    if (isStack) {
      await page.system.removeStock(stockUuid);
    } else {
      await page.system.editStock(stockUuid, { quantity: stock.quantity - quantity });
    }
